...
void loop()
{
  ...
  else if (digitalRead(record[2]) == HIGH)
  {
    ...
    {
      tone(buzzer, freq[remember[0][i]], (remember[1][i + 1] - remember[1][i]) * 100);
      delay((remember[1][i + 2] - remember[1][i + 1]) * 100);
    }
  ...
  for (int i = 0; i < 7; i++)
  {
    if (digitalRead(sw[i] == HIGH))
    {
      ...
      {
        remember[0][i] = i;
        remember[1][i] = millis() / 100 - start_time;

        while (digitalRead(sw[i])) tone(buzzer, freq[sw[i]]);
        noTone(buzzer);

        remember[0][i + 1] = i;
        remember[1][i + 1] = millis() / 100 - start_time;
        cnt += 2;
        Serial.print(i);
        Serial.print(" - ");
        Serial.println(cnt);
